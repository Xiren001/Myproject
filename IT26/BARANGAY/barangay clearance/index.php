<!DOCTYPE htmml>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Barangay clearance</title>
    <link rel="stylesheet" href="style.css">

</head>

<body id="body">

    <div class="container">
    <div id="cform">
        <div id="chform">


            <label>Full Name</label>
            <input type="text" id="A" name="username" required />

            <label>Age</label>
            <input type="number" id="B" name="userdetails" required />

            <label>Status</label>
            <input type="text" id="C" name="organisername" required />

            <label>ISSUED (month)</label>
            <input type="date" id="D" name="organisername" required />

            <label>ISSUED day of</label>
            <input type="date" id="E" name="organisername" required />

            <label>O.R No.</label>
            <input type="number" id="F" name="organisername" required />

            <label>Date Issuede:</label>
            <input type="date" id="G" name="organisername" required />

            <button id="bcertificate" onclick="check()">Generate</button>
            <button id="downloaddata" onclick="dcheck()">Print</button>
        </div>
    </div>



    
    <div id="main">
        <div class="cert-div">
            <div class="img-div">
                <img src="LOGO.png">
            </div>
            <div class="text-div">
                <p class="text-p">Republic of the Philippines</p>
                <p class="text-p">Province of Davao</p>
                <p class="text-p">Municipality of New Matina</p>
                <h1 class="brg-text">BARANGAY MATINA</h1>
            </div>
        </div>


        <div class="line"></div>
        <div class="line"></div>


        <div class="font-text">
            <h3>OFFICE OF THE BARANGAY CAPTAIN</h3>
            <h1>BARANGAY CLEARANCE</h1>
        </div>

        <div class="font1-text">
            <h3>TO WHOM IT MAY CONCERN:</h3>
        </div>


        <div class="font2-text">

            <div  class="text2p">
         
            <p>This is to certify that&nbsp;</p>
            <p id="text-1" class="font5"></p>
            <p>,&nbsp;</p>
            <p id="text-2" class="font5"></p>
            <p>&nbsp;years old,&nbsp;</p>
            <p id="text-3" class="font5"></p>
            <p>and a resident of Barangay San Vicente, Sto. Domingo, Albay, is known to be
            of good moral character and a law-abiding citizen in the community.

            </div>


            <div class="text2p">
            <p>To certify further, that he/she has no derogatory and/or criminal records filed </p>
            <p>in this barangay.</p>
            </div>

            <div class="text2p">
            <p><b>ISSUED</b> this&nbsp;</p>
            <p id="text-4" class="font5"></p>
            <p>&nbsp;day of&nbsp;</p>
            <p id="text-5" class="font5"></p>
            <p>&nbsp;2024 at Barangay San Vicente, Sto.</p>
            <p>Domingo, Albay, upon request of the interested party for whatever legal purposes it may serve.</p>
            </div>
        </div>



        <div class="font3-text">
            <h3>CRISTIAN LORIA</h3>
            <p>Barangay Captain</p>
        </div>


        <div class="font4-text">
            <div class="text5">
                <p>O.R No:&nbsp;</p>
                <p class="font5" id="text-6"></p>
            </div>
            <div class="text6">
                <p>Date Issued:&nbsp;</p>
                <p class="font5" id="text-7"></p>
            </div>
            <div class="text7">
                <p>Doc. Stamp:&nbsp; PAID</p>
            </div>
            
        </div>


    </div>

    </div>
    <script src="index.js"></script>
</body>

</php>